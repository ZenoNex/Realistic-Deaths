...
public class BlockMagmaBody extends BlockContainer{

	public BlockMagmaBody() {
		super(Material.rock);
		this.setBlockBounds(.1f, .1f, .1f, .9f, .8f, .5f);
	}
    	...
    	if(player.inventory.getCurrentItem() != null){
    		if(player.inventory.getCurrentItem().getItem() == ItemManager.CarvingKnife){
    			...
    			if(!par1World.isRemote){
    				...
    				RecipeManager.doManditoryDrops(player, EnumMobType.Magma);
	...
	public TileEntity createNewTileEntity(World var1, int var2) {
		return new TileEntityMagmaBody();
	}
