...
public class BlockCatBody extends BlockContainer{

	public BlockCatBody() {
		super(Material.rock);
		this.setBlockBounds(.3f, .05f, .2f, .69f, .4f, .57f);
	}
    	...
    	if(player.inventory.getCurrentItem() != null){
    		if(player.inventory.getCurrentItem().getItem() == ItemManager.CarvingKnife){
    			...
    			if(!par1World.isRemote){
    				...
    				RecipeManager.doManditoryDrops(player, EnumMobType.Cat);
