...
public class BlockVillagerBody extends BlockContainer{

	public BlockVillagerBody() {
		super(Material.rock);
		this.setBlockBounds(.2f, .1f, .3f, .8f, .9f, .95f);
	}
    	...
    	if(player.inventory.getCurrentItem() != null){
    		if(player.inventory.getCurrentItem().getItem() == ItemManager.CarvingKnife){
    			...
    			if(!par1World.isRemote){
    				...
    				RecipeManager.doManditoryDrops(player, EnumMobType.Villager);
	...
	public TileEntity createNewTileEntity(World var1, int var2) {
		return new TileEntityVillagerBody();
	}
