...
public class BlockEndermanBody extends BlockContainer{

	public BlockEndermanBody() {
		super(Material.rock);
		this.setBlockBounds(0.2f, .1f, 0f, 
				.8f, .8f, .6f);
	}
    	...
    	if(player.inventory.getCurrentItem() != null){
    		if(player.inventory.getCurrentItem().getItem() == ItemManager.CarvingKnife){
    			...
    			if(!par1World.isRemote){
    				...
    				RecipeManager.doManditoryDrops(player, EnumMobType.Enderman);
	...
	public TileEntity createNewTileEntity(World var1, int var2) {
		return new TileEntityEndermanBody();
	}
