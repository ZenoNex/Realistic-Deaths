...
public class BlockChickenBody extends BlockContainer{

	public BlockChickenBody() {
		super(Material.rock);
		this.setBlockBounds(.36f, .1f, .2f, 
				.65f, .6f, .54f);
	}
    	...
    	if(player.inventory.getCurrentItem() != null){
    		if(player.inventory.getCurrentItem().getItem() == ItemManager.CarvingKnife){
    			...
    			if(!par1World.isRemote){
    				...
    				RecipeManager.doManditoryDrops(player, EnumMobType.Chicken);
	...
	public TileEntity createNewTileEntity(World var1, int var2) {
		return new TileEntityChickenBody();
	}
